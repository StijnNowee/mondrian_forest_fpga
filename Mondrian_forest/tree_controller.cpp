...
void tree_controller(hls::stream<input_vector> &splitFeatureStream, hls::stream<FetchRequest> &feedbackStream, hls::stream<FetchRequest> &fetchRequestStream, hls::stream<int> &processTreeStream)
{
    std::cout << "Read from input..." << std::endl;
    input_vector input = splitFeatureStream.read();
    FetchRequest newRequest{input, 0, 0, false, false, false};
    fetchRequestStream.write(newRequest);
    FetchRequest request = feedbackStream.read();
    while(request.needNewPage){
        fetchRequestStream.write(request);
        request = feedbackStream.read();
    }
