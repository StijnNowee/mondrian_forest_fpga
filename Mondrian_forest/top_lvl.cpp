...
void top_lvl(
   ...
   #pragma HLS BIND_STORAGE variable=nodePool type=ram_1p
   #pragma HLS BIND_STORAGE variable=treePool type=ram_1p
    ...
    for (int i = 0; i < TREE_COUNT; i++){
        #pragma HLS UNROLL
        train(inputFeatureStream, inputLabelStream, rngStream, treeInfoStreams[i], treeOutputStreams[i], nodePool);
    }
